...
const MenuItems = ({
  ...
  handleMouseMove: (section: string | null) => void;
  isTheLab: boolean;
  hasAppeared: boolean;
...
const Header = () => {
  ...
  const handleMouseMove = (section: string | null) => {
    switch (section) {
      case 'lab':
        setClipMaskValue({ start: '83%', end: '0%' });
        break;
      case 'email':
        setClipMaskValue({ start: '67%', end: '17%' });
        break;
      case 'github':
        setClipMaskValue({ start: '50%', end: '33%' });
        break;
      case 'linkedin':
        setClipMaskValue({ start: '33%', end: '50%' });
        break;
      case 'codepen':
        setClipMaskValue({ start: '17%', end: '67%' });
        break;
      case 'theme':
        setClipMaskValue({ start: '0%', end: '83%' });
        break;
      default:
  ...
  return (
    <nav className="fixed bottom-4 left-1/2 transform -translate-x-1/2 z-20 bg-background shadow-lg rounded-xl px-1 gap-2 flex items-center border border-border overflow-hidden">
      <MenuItems
        ...
        className="flex items-center bg-primary text-background dark:text-foreground absolute left-1 right-1 top-0 bottom-0 motion-safe:transition-[clip-path,background-color] motion-safe:[transition-duration:150ms] ease-in-out-quad pointer-events-none"
        style={{
